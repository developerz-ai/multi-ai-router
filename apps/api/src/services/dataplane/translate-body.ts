...
/**
 * The upstream body a translate candidate sends — built lazily, and at most once per target
 * dialect.
 *
 ...
 * to `k3` on the next — and the request body is otherwise identical between them, so the model is
 * the only field that has to move on a failover.
 */
...
export interface TranslatedRequestBody {
   ...
   */
  bodyFor(pair: TranslationPair, upstreamModel: string): Uint8Array
}
...
export function createTranslatedRequestBody(
  ...
  const converted = new Map<Dialect, Record<string, unknown>>()
  let source: unknown
  let parsed = false
  ...
  return {
    bodyFor(pair, upstreamModel) {
      let translated = converted.get(pair.egress)
      if (translated === undefined) {
        ...
        const result = pair.request(body(), context)
        ...
        converted.set(pair.egress, translated)
      }
