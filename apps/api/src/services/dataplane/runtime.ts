...
export interface DispatchRuntime extends RuntimeInput {
   ...
   * recorded** — the caller closes its own span before calling, because only the caller knows when
   * its attempt started and stopped waiting: the span opens after the body was converted, and a
   * stream settles well after the chain returned.
   *
