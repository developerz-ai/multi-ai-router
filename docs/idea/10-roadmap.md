...
| **M4 — Claude subscriptions via the Agent SDK** | The `claude` CLI in the image, one `CLAUDE_CONFIG_DIR` provisioned per Account on a persistent volume, CLI-driven login with paste-back capture from the admin UI, requests served through `@anthropic-ai/claude-agent-sdk`'s `query()`, SDK output re-synthesized into Anthropic and OpenAI wire format, `rate_limit_event` quota ingestion, `needs_reauth` on failure (including the 30-day refresh-token expiry, found by a daily free `claude auth status` sweep over every subscription and classified on the request path), on-disk SDK transcripts swept after `RETENTION_SDK_TRANSCRIPT_HOURS`. **This is the hardest milestone — scope it as such.** Nothing here is a proxy hop: it is a subprocess, a config directory, and a protocol rebuilt from a different shape. | A Claude Max subscription can be connected from the UI, several subs coexist with no cross-contamination, a streaming Claude request through the SDK is indistinguishable on the wire from a passthrough one, quota events drive `cooling_down`, and a broken login lands in `needs_reauth` (not a 500). |
...
| **M7 — Admin UI** | SolidJS SPA, overview, accounts **grouped by provider** (problem groups first; collapsible sections with count, routable-now, worst status, next login expiry), pools, keys, usage totals and charts, the reset/health surface (per-window countdowns with the provider's gauge reading labelled *reported*, `exhausted` banner, **Re-check now**, **Test now**, **Discover models**), the subscription-login surface (each Claude sub's login shown as *valid until* + countdown, warn at 7 days, danger at 2, a dashboard banner a week ahead, and **Reconnect all** — one guided sequence of the ordinary connect dialog, skip and next per account), settings. Fleet reads (accounts, pools) poll every `LIVE_POLL_MS` while the tab is visible; every mutation invalidates its readers on settle, success or not. | The whole operator experience is the browser — no config file, no CLI, no DB surgery. "When does it come back?" and "is it back yet?" are both answerable without reading logs, "does this credential actually work?" is one confirmed button press away, and "what does this account serve?" is one free one. |
...
| **Router-side MFA after OIDC** | MFA enrollment and recovery now belong to the configured IdP, which can enforce them before issuing an ID token. A second router-held factor on top of that session would add another recovery and secret lifecycle; build it only if an operator needs defense independent of the IdP. |
