...
| How built-ins are disabled | An **explicit allowlist that names the tools permitted to run** — never a blocklist, never a default, never "we don't pass tools so it won't". A blocklist fails open the day the SDK ships a new built-in; a default is not a decision. The allowlist is one reviewed constant, and adding a name to it is a security change |
