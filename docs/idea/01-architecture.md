...
| Credential sweep + idle-account keepalive | daily; the billed half against a `IDLE_ACCOUNT_AFTER_DAYS` threshold | Two halves. The **free** one runs every tick over *every* account holding a CLI-managed credential: `claude auth status`, no provider contact, so an expired Claude subscription reads `needs_reauth` within a day instead of on a client's failed request. The **billed** one is the only task here that spends money: the Agent SDK refreshes a subscription's *access* token only when it runs, so an account traffic has forgotten gets one real request. What neither can do: a subscription's refresh token hard-expires ~30 days after login however much it is used — only a re-login moves that, and the operator reconnects monthly |
...
| Agent-SDK transcript sweep | every `SDK_TRANSCRIPT_SWEEP_INTERVAL_MINUTES`, jittered | Removes the `<session>.jsonl` and `<session>/` artifacts the `claude` CLI leaves under each Account's `CLAUDE_CONFIG_DIR` once older than `RETENTION_SDK_TRANSCRIPT_HOURS`. Bounded batch, oldest first, pure plan; only those two shapes are ever candidates in a directory that also holds live credentials. A resume onto a swept transcript is a `stale-session` replay, never a failed request |
