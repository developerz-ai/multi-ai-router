...
On success the callback sets the cookie and sends the browser to `/`. The hop is client-side, not a `302`, and for the same reason the route is unguarded: this response is the tail of a cross-site chain that began at the provider, so a server-issued redirect can produce a follow-up request that arrives *without* the cookie just set — the console would bounce to `/login` and read as a failed sign-in. A navigation issued by the callback document itself is same-origin and self-initiated, which carries a Strict cookie. A visible link covers the no-JS case.
...
### Session lifetime and durability

Sessions live in Postgres (`admin_sessions`, `services/admin-auth/postgresSessionStore.ts`), so a redeploy, a crash, or a `bun --watch` restart no longer logs the operator out — the row is what authenticates, and a new process reads it back on the first request. The row stores a **SHA-256 hash** of the opaque session id, never the id: the cookie value is the bearer, and a database dump must not be a bag of live sessions. The CSRF token is stored as-is because the SPA reads it back on every session read and it authorizes nothing without the cookie beside it. A small per-replica read-through cache (`ADMIN_SESSION_CACHE_MAX`) keeps `authenticate()` off Postgres for sessions it has seen; the idle-window slide is written back at most once per `ADMIN_SESSION_TOUCH_INTERVAL_SECONDS`, fire-and-forget, so an admin request never waits on the write. Logout deletes the row synchronously. Expired rows are swept by the `admin_session_purge` task.

**The defaults are thirty days on both bounds** (`ADMIN_SESSION_IDLE_MINUTES=43200`, `ADMIN_SESSION_ABSOLUTE_HOURS=720`), and that is a deliberate trade for a single-operator console behind an IdP: the previous eight-hour idle window and daily cap meant a login on every redeploy and every morning, which for one operator is friction with no compensating threat model. What the trade costs: a stolen cookie is a live credential for up to thirty days of inactivity. What still holds: the cookie is `HttpOnly`/`Secure`/`__Host-` so a script or a sibling host cannot read or plant it; logout is a real invalidation, not a client-side forget; a login always mints a fresh id and CSRF token (no fixation); the IdP's own session is untouched and can be revoked there; and both bounds are configuration — an operator who wants the old posture sets `480` and `24`. The cookie's `Max-Age` follows the idle window, so the browser forgets it on the same schedule the server does.
