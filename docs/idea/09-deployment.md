...
| `CLAUDE_SDK_CREDENTIAL_KEEPALIVE` | no | `true` | Gives a logged-in subscription account whose **access** token is about to expire one small turn to refresh it. Only a real turn refreshes — the turn-free handshake behind the model-catalog sweep and the usage gauge leaves `.credentials.json` untouched — so without this an account nothing routes to goes cold, and the next thing to touch it finds a refresh the upstream will not honour ([11-anthropic-agent-sdk.md §3](11-anthropic-agent-sdk.md)). Costs one small turn per cold account per sweep. `false` logs the cold credential instead of warming it. |
| `CLAUDE_SDK_CREDENTIAL_KEEPALIVE_BEFORE_MINUTES` | no | `IDLE_ACCOUNT_PROBE_INTERVAL_MINUTES` + 30 | How much margin before expiry counts as cold. **Derived from the sweep interval rather than fixed**, because a margin narrower than the gap between sweeps leaves a token that expires between two ticks unwarmed by either: with a 6 h sweep and the flat hour this first shipped with, an account expiring 2 h 49 m out was skipped now and already dead by the next tick. `0` waits until the token has actually expired. |
| `CLAUDE_SDK_CREDENTIAL_REFRESH_SKEW_SECONDS` | no | `300` | How near an access token's expiry counts as the refresh window, inside which only one `claude` subprocess per Account may run. The refresh token **rotates when spent**, so two subprocesses crossing that moment double-spend it and the loser's CLI blanks the credential file — a permanent deauthentication ([11-anthropic-agent-sdk.md §3](11-anthropic-agent-sdk.md)). The first caller is never delayed; outside the window this costs one cached read. `0` is legal and means *no lead time* — the window still guards, but only once the token has actually expired. |
