...
| A revoked or edited router key still authenticates for up to a minute, on some but not all replicas | Verified keys are cached in memory per process (`DEFAULT_KEY_CACHE_TTL_MS`, 60s) so verification does not round-trip Postgres on every request; admin revoke/edit calls `invalidate()` on the replica that served the request, not the others | Expected, bounded staleness — a revoked key stops working everywhere within the cache TTL even with zero replica coordination, since a compromised key is not a reason to add a broker (non-negotiable: background work is in-process only, no Redis/pub-sub). If a key must be dead **immediately** on every replica, there is no faster path today than the TTL; treat that as a real limit when sizing `ADMIN_SESSION` / key-cache TTLs for a multi-replica deployment. See [04-api-keys-and-access.md](04-api-keys-and-access.md#verification-path). |
