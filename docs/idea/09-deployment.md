...
| `IDLE_ACCOUNT_AFTER_DAYS` | no | `7` | How long an account must go unused before one real, **billed** request is spent keeping it alive. Exists because a Claude subscription's tokens are refreshed by the Agent SDK *only when it runs* — the access token lasts hours and the refresh token weeks, so an account nobody routes to expires silently and fails at the moment it is next needed. Keep this well inside the shortest refresh-token life among your accounts; raising it past that re-opens the hole. **`0` refused at boot**: every account would be idle, so every account would be billed every tick. |
