...
| `CLAUDE_SDK_CREDENTIAL_KEEPALIVE_BEFORE_MINUTES` | no | `60` | How much margin before expiry counts as cold, so a credential is warmed on the tick *before* it dies. `0` waits until the token has actually expired. |
