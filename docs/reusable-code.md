...
| `upstreamUrl(driver, account, dialect)`, `upstreamModelsUrl(…)`, `upstreamCountTokensUrl(driver, account)` | `services/dataplane/egress/endpoint.ts` | Where a request lands on an Account's endpoint: the driver owns the base URL, the dialect owns the path below it. The count-tokens builder takes no dialect — only `anthropic` states that endpoint at all |
