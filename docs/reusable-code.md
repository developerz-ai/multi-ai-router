...
| `AccountBilling`, `DEFAULT_ACCOUNT_BILLING` | `domain/account.ts` | How an Account is billed, which decides how its usage is priced and nothing else. An **Account** property, never derived from the provider id — the same provider sells both a metered API and a flat-fee plan. The default a provider does not override, and what migration `0014` backfills |
...
| `PG_MAX_BIND_PARAMETERS`, `USAGE_RECORD_BIND_PARAMETERS_PER_ROW`, `USAGE_RECORD_MAX_BATCH_ROWS` | same | Sizing a usage batch. `insertMany` spends one bind parameter per column it sets and Postgres binds 65 535 per statement, so a batch past `USAGE_RECORD_MAX_BATCH_ROWS` is rejected on every flush — forever, and the recorder's one retry cannot save a batch whose size is what the database objects to. `config/env.ts` refuses such a `USAGE_BATCH_SIZE` at boot; do not re-derive the arithmetic anywhere else |
| `createUsageReadRepository(db)` → `UsageReadRepository` | `repositories/usage-read-repository.ts` | Aggregates over raw `usage_records` — totals, breakdowns, buckets, percentiles, and `outcomes(window)` for the failure split. Requests are `count(distinct correlation_id)`, attempts are `count(*)`; never sum rows for "requests". `outcomes` is deliberately unordered and raw-only: the rollup has no per-outcome grain, and which failure to read first is a display decision a pure function owns |
...
| `createUsageDailyRepository(db)` → `UsageDailyRepository` | `repositories/usage-daily-repository.ts` | The daily rollup, both directions: `rollupDay(day)` replaces exactly one whole UTC day (the batch — a caller walking a window loops), `deleteOlderThan` sweeps by day, `totals`/`breakdown` read them. Any aggregate over a **closed** day belongs here — raw rows expire, these do not |
...
| `createScheduledTaskRepository(db)` → `ScheduledTaskRepository` | `repositories/scheduled-task-repository.ts` | The scheduler's run log. `begin`/`finish` bracket a tick; `lastRun` is the health read, `lastSuccess` is the cursor a catch-up task resumes from — a task reading `lastRun` inside its own `run` reads itself; `deleteOlderThan` is the janitor's sweep of the log, and never takes a run with no `finishedAt` |
...
| `redact(fields)`, `redactValue(s)`, `isSecretFieldName(name)`, `REDACTED` | `logging/redact.ts` | Anywhere a value could carry credential material. The logger already applies it, to `msg` as well as to the fields. Two lists: secret field names/markers (matched with `-`/`_` stripped, so `api_key`/`apiKey` are one entry), and self-identifying value shapes (router keys, `sk-`, `Bearer`/`Basic`, JWTs, URL userinfo, query-string keys, `AIza`/`gh*_`/`xai-`/`gsk_`/`csk-`). Fails closed past four levels of nesting; unwraps `Error`/`Map`/`Set` rather than serializing them as `{}`. A new provider credential shape belongs here, not in a second scrubber |
| `toErrorResponse(error, dialect)`, `notFoundResponse(dialect)`, `renderErrorBody(...)`, `dialectForPath(path)` | `errors/render.ts` | Rendering a failure. Pure — no Hono, no I/O. The dialect shapes the **body**, never the status. `renderErrorBody` is the one choke point every client-facing message leaves through, and it runs `redactValue` there rather than at each of its callers |
| `parseEnv(raw)`, `Env`, `EnvValidationError`, `decodeEncryptionKey(value)`, `LOG_LEVELS`, `ENV_FIELDS` | `config/env.ts` | Reading configuration. Pure over a raw map; `main.ts` owns the one `process.env` read |
| `atLeastOne`, `wholeNumber`, `fraction`, `flag`, `nonEmpty`, `absoluteUrl`, `usageBatchSize`, `encryptionKey`, `ZERO_IS_LEGAL` | `config/fields.ts` | Adding an environment variable. The field vocabulary the schema is built from, and the zero policy: a numeric knob takes `atLeastOne` unless zero is a setting an operator could mean, in which case it takes `wholeNumber` **and** an entry in `ZERO_IS_LEGAL` saying what zero does. A drift guard walks the schema and fails the build on anything that is neither |
...
| `createLifecycle()` → `Lifecycle` | `services/shutdown/lifecycle.ts` | Anything that has to know a shutdown started. One latch, two readers: `/readyz` answers `503 shutting_down` from it, and the signal handlers use `begin()`'s `false` to recognise a second SIGTERM instead of running the flush twice. Two booleans would drift — readiness would keep answering `ready` through a drain nobody told it about |
...
| Thing | Where | Use it when |
|---|---|---|
| `estimateCost(input: CostInput)` → `CostEstimate` | `services/cost/estimate.ts` | Pricing an attempt. `CostInput` is `{ provider, model, tokens, billing?, prices? }` — `model` is the **upstream** name after the alias map, `billing` is the Account's mode (defaults to `metered`), and `prices` is any `RateLookup`, which omitted prices off the shipped table. Pure — no clock, no store — and the **only** place a `costBasis` is decided: `metered`, `notional` for a subscription Account's attribution, `unknown` when unpriced. Unknown is null, never zero |
| `rates(input, output, cache?)` → `RateCard`, `tiered(standard, fromPromptTokens, long)` → `ModelRates`, `modelLookupKeys(model)`, `RateCard`, `LongContextTier`, `ModelRates`, `ModelTable`, `RateLookup`, `ShippedRate` | `services/cost/rates.ts` | Writing or reading a vendor price table. `rates` states the two absent-price defaults once — no published cached-input price bills at the full input rate, no published cache-write price is zero — so a table that omits them says the same thing everywhere. `tiered` attaches the long-context card that **replaces** the standard one above a prompt size, never a surcharge on the excess. `modelLookupKeys` is the one place a dated snapshot folds onto its family; the override book normalizes through it too, so an override written for a family answers for the pin |
| `lookupRates(provider, model)` → `ModelRates \| null`, `listShippedRates()`, `PRICE_TABLE_AS_OF` | `services/cost/prices.ts` | Reading a shipped per-Mtok rate, enumerating the whole table (flattened one row per tier) for the settings screen, or dating it. Twelve vendor tables under `services/cost/tables/`, each commented with its provenance and blast radius; four providers ship no price by design (`openrouter`, `ollama`, the two `*-compatible`) and price as `unknown`. **`PRICE_TABLE_AS_OF` is updated in the same commit as any edit under `tables/`, never without one** — it is what `GET /api/admin/settings` and `router_price_table_asof_timestamp_seconds` report |
...
| `planConfigDirReap(input)` → `ReapPlan` | `scheduler/tasks/config-dir-reap.ts` | Deciding which `CLAUDE_CONFIG_DIR`s on the volume no Account claims. Pure — the survey, the live id set, the instant and the grace are all injected — so the two failures it sits between (a credential nobody rotates vs a working subscription logged out for good) are both pinned without a disk |
...
| `createUsageRecorderFromEnv(deps)` → `UsageRecorder` | `services/usage/fromEnv.ts` | Building the production recorder: repository writer, queue shape from env, and the throttled log lines that keep a shed record or a rejected batch from being silent. A batch going back for its retry is a `warn`, the same batch refused twice is an `error`, and the database's own complaint is quoted bounded — a usage statement is thousands of bind parameters long. `composition.ts` is its one caller; tests use `createUsageRecorder` with an array |
| `createUsageRecorder(writer, options)` → `UsageRecorder`, `UsageStats`, `UsageWriteFailure` | `services/usage/recorder.ts` | Writing facts about the past off the request path. `record()` never awaits; the drain batches. A refused batch gets **one** retry on the next flush — held beside the queue so overflow cannot shed it first, not re-observed (`onRecord` feeds metrics, and twice would be an attempt the router never made), then discarded rather than looped on forever. `stats()` is what `router_usage_write_failures_total` is fed from; a failure that only moves a counter nobody exports is a failure nobody sees |
...
| `createOpenAiChatToolCallReader()` → `.key(call)` | `services/translate/shared/openai-chat-tool-calls.ts` | Reading `delta.tool_calls[]` off a streamed openai-chat chunk. The `index` is required by the schema and is not always sent (LM Studio, Ollama), so keying by the entry's position inside its own chunk collapses distinct calls into one. Tells a *new* unkeyed call from a *continuation* of one, which is the part Responses' own ordinal counter never has to do |
| `openAiChatReasoningSchema`, `readOpenAiChatReasoning(source)` | `services/translate/shared/openai-chat-reasoning.ts` | Reading a reasoning model's thinking off an openai-chat `delta` or `message`. OpenAI publishes no such field, so it is a vendor extension with two live spellings — `reasoning_content` (DeepSeek, SGLang, GLM, Qwen, xAI) and `reasoning` (OpenRouter, Groq, Ollama) — and vLLM has emitted each in turn. Spread the schema fragment into a pair's own reader and take the precedence rule from here; a second copy is the thing that would silently disagree. Read only, never emitted — see `06-protocol-translation.md#known-lossy-edges` |
| `createPendingToolCalls()` | `services/translate/shared/pending-tool-calls.ts` | Emitting toward a dialect that holds one open block or item at a time, from one that does not. openai-chat may revisit an earlier `tool_calls[].index`, so a second call cannot displace the first: hold it here and give it a block of its own when the live one closes. Dropping it instead ships a `tool_use` with truncated `input` under a `stop_reason` saying it was complete |
| `assertStatelessResponses`, `assertPlainResponseFormat` / `assertPlainTextFormat`, `assertNoStopSequence`, `assertTranslatableToAnthropic`, `rejectField` | `services/translate/shared/reject.ts` | Refusing a request field a target dialect cannot carry. **Every refusal lives here, once**, because a field with two spellings (`response_format` / `text.format`, `stop` / `stop_sequences`) needs one rule — two copies could disagree about which requests are servable. Never interpolates a value out of the body: a refused request may carry anything and an error body is client-facing. A field silently dropped instead is answered as a different question, so the rule is refuse, never degrade |
...
| `createSdkInvoker(deps)` → `SdkInvoker`, `SdkQueryFn` | `providers/claude-sdk/invoker.ts` | The implementation behind that seam, and the only place all of the pieces below are held at once. Reads the body **once**, builds the prompt from the lineage plan, takes a concurrency slot, launches, and renders. Three lifetimes are its own: the slot and the abort bridge are released when the *message stream* ends — not when `query()` returns, which is immediately — and the session is reported once, at the end, with the assistant uuid an undo needs. `query()` and the executable ladder are injected, so the transport is exercisable without a subprocess |
| `readSdkRequest(body)` → `SdkRequest` | `providers/claude-sdk/request.ts` | The one parse of an Anthropic Messages body the SDK path performs — prompt, tools, system prompt, and response shape out of a single pass, which is what makes the labeled exception to "never parse a passthrough body" affordable. An unreadable body is an empty request, never an error |
| `buildSdkPrompt(input)` → `PromptBlock[]` | `providers/claude-sdk/prompt.ts` | The conversation as the one user turn `query()` is asked. A resume sends the delta and only the delta; anything the SDK has not held is a **framed** replay, because a flat transcript teaches the model to continue both speakers and to write tool calls as prose. Text and images survive as real blocks even inside the frame; thinking is dropped rather than replayed unsigned. Pure — the resume-or-replay decision was already made by `session/lineage.ts` |
| `createQueryLaunch(input)` → `QueryLaunch`, `MAX_TURNS` | `providers/claude-sdk/options.ts` | Launching one `query()`. Returns the `Options` plus `abort()`/`detach()`, because the SDK wants an `AbortController` while the data plane produces an `AbortSignal` — and a bridge listener on a signal that outlives the query is a leak. Every isolation field (`settingSources: []`, `strictMcpConfig`, `skills: []`, `tools: []`) reads as dead code because its effect is an absence; none may be deleted as cleanup. The `SessionPlan` becomes `resume`/`forkSession`/`resumeSessionAt` here, verbatim, and `fresh` is the absence of all three |
...
| `createClaudeConnectService(deps)` → `ClaudeConnectService`, `ClaudeConnectMode` | `services/accounts/connect/claude.ts` | Connect and reconnect for a Claude subscription. Owns what the CLI does not: the `state` read back out of the authorize URL, one-shot (a wrong paste burns the login), TTL from `env.retention.oauthStateMinutes`, bound to one Account row. Pending logins are in memory because a pending login *is* a running subprocess, and all three calls take the Account's turn so no two of them can hold one directory at once. Clears `needs_reauth` and nothing else — a disabled Account stays disabled. `mode` only selects the audit kind: connect vs reconnect is declared by the operator, never detected |
| `createAccountTurns()` → `AccountTurns` | `services/accounts/connect/turns.ts` | Serializing calls that read one Account's in-memory state and then replace it across an await. A **queue**, not the single-flight in `refresh/refresher.ts`: the second caller must run, and run after, so it can see and cancel what it displaces — which is what makes "a second `begin` supersedes the first" true of the subprocess and not just the map entry. Per account, so one login's handshake never makes another Account wait; a failed turn rejects its own caller only, and a drained queue drops its entry |
...
| `foldFailures(counts, stitchedAttempts)` → `UsageFailures`, `EMPTY_FAILURES` | `services/usage-read/failures.ts` | Taking the error rate apart. Pure. `quota_exhausted` and `credits_exhausted` stay two numbers — folding them into one "capacity" figure is the non-negotiable-7 conflation. The denominator is the scan's own, never the stitched total, and `partial` says when the counts are a floor rather than a total |
...
| `createAccountConfigDirs(options)` → `AccountConfigDirs`, `ConfigDirEntry`, `ConfigDirFs`, `ConfigDirError`, `CONFIG_DIR_MODE` | `providers/claude-sdk/config-dir.ts` | Naming, creating, destroying, and surveying an Account's `CLAUDE_CONFIG_DIR`. `pathFor` is pure and keyed on Account id; `provision`/`remove` are idempotent and bounded to `<root>/<id>`, so neither can reach a path the router did not mint. `list()` reports every directory under the root with its name classified (account id or not) and its newest timestamp, reading nothing inside one and following no symlink. The filesystem is injected (`ConfigDirFs`), so a test asserts which directory an Account got without touching disk. Constructed once at boot — a root that is relative, or at/under the CLI's own `~/.claude`, throws there rather than at first use |
...
| `upstreamUrl(driver, account, dialect)`, `upstreamModelsUrl(…)`, `upstreamCountTokensUrl(driver, account)`, `upstreamEmbeddingsUrl(driver, account)` | `services/dataplane/egress/endpoint.ts` | Where a request lands on an Account's endpoint: the driver owns the base URL, the dialect owns the path below it. Neither operation-specific builder takes a dialect, for opposite reasons — only `anthropic` states the token count at all, and both OpenAI surfaces state embeddings at the same place. `upstreamModelsUrl` is the admin plane's, not the data plane's: `GET /v1/models` is answered from the warm catalog, and the only caller is **Discover models** |
| `resolveModel(account, requestedModel)` → `ModelResolution`, `advertisedModels(account)` | `services/routing/model.ts` | Whether a name a client sent is served, and the names for which that answer is yes. Two sides of one map: `supportedModels` is upstream-side, alias *keys* are requested-side, so support is judged after the rename. `advertisedModels` is a filter over `resolveModel` rather than a union of the two fields — that is what makes `GET /v1/models` and selection unable to drift, in a codebase where a listing promising a `503` is the failure mode |
| `createDiscoverModelsService(deps)` → `DiscoverModelsService` | `services/accounts/discover-models.ts` | Asking an Account's upstream what it serves and writing the answer into `supportedModels`. One `GET` through `runAttempt`, so header rules, credential handling and failure classification are the live path's. Both dialects' listings are read from the one field they share (`data[].id`); an empty or unreadable answer writes **nothing**, because `[]` would eventually read as "serves nothing". The write goes through `AccountsService.update`, so the warm catalog refresh and the field audit come free |
| `isWindowSpent(window, now, threshold?)`, `findSpentWindow(account, now, threshold?)`, `continuousHeadroom(account)`, `earliestReset(windows)`, `mergeQuotaWindows(held, incoming)` | `services/routing/quota.ts` | Any "is this account out of quota" or "which has the most left" question. Two jobs kept apart by `utilizationSource`: *any* signal answers spent, only a **continuous** one ranks headroom, and `quota-aware` reports `null` rather than ranking on an absence. `continuousHeadroom` reads named quota windows **and** `AccountSnapshot.limiterWindows` — the HTTP limiter headers are the fleet's only continuous reading, and skipping them ranks nothing on every API-key account there is. `mergeQuotaWindows` folds two sets into one — per kind, never wholesale (a turn reporting `five_hour` says nothing about `seven_day`), fresher `lastCheckedAt` wins (another replica's reading, or the quota floor retiring an expired window, is not undone by a stale in-memory copy), and `undefined` leaves the set alone where `[]` would claim the account holds none |
...
| `unservableError({plan, decision, capable, now})` | `services/dataplane/unservable.ts` | Which of two explanations answers a request that found candidates and could plan none. Pure. Selection drops an account for its health, planning for its shape, and neither sees the other — so a health verdict on an account that *could* have served the operation (`capable`) outranks the plan's "no account can do this". A cooling Anthropic account beside a healthy OpenAI one makes `count_tokens` a `429` with `Retry-After`, not a `503` naming a gap the operator already filled. Delegates the status rules to `routing/no-candidates.ts` rather than restating them |
...
| `admitHalfOpenProbe(health, candidate, now)` → `HalfOpenProbe`, `HealthStore.admitProbe` / `releaseProbe` | `services/dataplane/probe.ts`, `health.ts` | Anywhere a half-open candidate is about to be dispatched to. The compare-and-swap that makes "one request through as a probe" true rather than aspirational: taken at attempt time so an unreached probe holds nothing, released on the attempt's verdict, and visible in the next health snapshot so the pure filter drops everyone else as `probe-in-flight` — a `429` with the hold's expiry, never a `500` and never a queue. Per account. The operator's **Re-check now** joins it through `HealthStore.reset` rather than adding a second recovery path |
| `foldRateLimit(state, signal, now, breaker)`, `HealthStoreOptions.onQuotaWindows` / `onBlocked` / `onReset` | `services/dataplane/health-reading.ts`, `health.ts` | What one response's rate-limit reading does to an Account: record it always, never let it overwrite a terminal verdict, otherwise cool down through the breaker's own transition. Pure, so the precedence rule that keeps `exhausted` from becoming a countdown is testable without a store. `onQuotaWindows` is the one seam the durable quota writer hangs off — both transports fold here and nowhere else; `onBlocked` is its twin for the breaker, firing once per *transition* into a status no clock undoes and reporting every block it forms (the persistence policy lives downstream, not here); `onReset` is how "drop every mark" also drops the Agent SDK's own per-Account buckets and any queued status verdict |
| `createQuotaWindowWriter(deps)` → `QuotaWindowWriter` | `services/dataplane/quota-writer.ts` | Making an observed quota reading durable. `record` is a map write and returns; the upsert happens on `QUOTA_WRITE_INTERVAL_MS`. A **map, not a bounded queue** — a window is state, so the newest reading supersedes the older one and there is nothing to shed; reach for `usage/recorder.ts` when the thing written is a fact about the past. Never a scheduled task: the advisory lock would persist the lock-holder's readings and drop every other replica's |
| `createAccountStatusWriter(deps)` → `AccountStatusWriter`, `persistable(status)`, `OVERWRITABLE_BY_OBSERVATION` | `services/dataplane/status-writer.ts` | Making the breaker's own verdict durable — the same three properties as the quota writer, for the same reasons, on `ACCOUNT_STATUS_WRITE_INTERVAL_MS`. What is *not* shared is the policy: only `exhausted` and `needs_reauth` are stored (a cooldown is clock-recoverable and a stored countdown is stale immediately; a `disabled` formed from a bad API key would be indistinguishable from an operator's switch), and the `UPDATE` is guarded by `AccountRepository.updateStatusWhen` so an observation can never overwrite `disabled` or a block already recorded. `forget` is wired to `HealthStore.reset` so a queued verdict cannot land after the operator's **Re-check now** |
| `relaySuccess(ctx, servable, attempt, response, at)`, `recordAttemptFailure(...)`, `AttemptClock`, `AttemptRelay` | `services/dataplane/chain-relay.ts` | What one attempt leaves behind — the bytes the client gets and the `UsageRecord` that says what it cost. Split from the chain because a success settles long after the loop returned, and because every attempt writes a row including every failure. `ChainContext` satisfies `AttemptRelay` structurally, so nothing is threaded |
...
| `failureClassFor(outcome)`, `groupFailures(counts)`, `FAILURE_CLASSES`, `HEADLINE_FAILURE_CLASSES` | `lib/failure-classes.ts` | Turning a failed attempt into the remedy it implies — a status plus one line an operator can act on. Finer than core's `UsageFault` **on purpose**: `quota_exhausted` and `credits_exhausted` are both `capacity` and their remedies are opposites. It may split a fault, never contradict one, and a unit test asserts that against core. Colour is *not* decided here — every class carries the `fault` `faultToken` already maps |
...
| `parseModelList`, `formatModelList`, `parseModelAliases`, `formatModelAliases` | `lib/account-models.ts` | An account's model configuration as an operator types it: the comma-separated served list, and the `requested = upstream` alias map. **The alias map's two sides are not interchangeable** — a key is what a client sends, a value is the name that goes upstream (`services/routing/model.ts`) — so both dialogs read one parser rather than each spelling the direction out |
| `BILLING_OPTIONS`, `billingLabel`, `billingConsequence` | `lib/billing.ts` | Wording an Account's billing mode anywhere in the console — both dialogs and the accounts table. `billingConsequence` states what the setting *does* rather than what it is called: "metered vs subscription" names no column to an operator, and a surface that spelled the consequence its own way would describe two different things |
| `toDateTimeInput`, `fromDateTimeInput` | `lib/datetime-input.ts` | Any `<input type="datetime-local">` bound to an ISO-8601 field. The control speaks local wall time and the API speaks an offset; converting by hand moves the instant by the operator's UTC offset and drifts again on every round-trip |
...
| Whether a provider is "a subscription" | `ProviderDriver.billing`, surfaced as `defaultBilling` / `billingFixed` on the descriptor and applied by `resolveBilling` (`services/accounts/rules.ts`). A hardcoded set of subscription provider ids — which the cost estimator used to hold — answers "is this sold *only* as a subscription" and "is this account on a plan" with one lookup, and gets the second wrong for every vendor selling both. The second question is the Account's `billing` column and nothing else |
| The zero policy for a numeric env var (`ZERO_IS_LEGAL` in `config/fields.ts`) | A knob added as `wholeNumber` without a thought about zero is a kill switch that boots clean: the sweep empties its table, the interval re-arms at 1 ms, the cache answers nothing. One list, walked by one drift guard — a second judgement made inline in the schema is a knob nobody checked |
