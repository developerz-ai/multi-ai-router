...
| `PG_MAX_BIND_PARAMETERS`, `USAGE_RECORD_BIND_PARAMETERS_PER_ROW`, `USAGE_RECORD_MAX_BATCH_ROWS` | same | Sizing a usage batch. `insertMany` spends one bind parameter per column it sets and Postgres binds 65 535 per statement, so a batch past `USAGE_RECORD_MAX_BATCH_ROWS` is rejected on every flush — forever, because a refused batch is never re-queued. `config/env.ts` refuses such a `USAGE_BATCH_SIZE` at boot; do not re-derive the arithmetic anywhere else |
