...
| `findAuthorizeUrl(output)`, `readState(url)`, `parsePastedCode(pasted)`, `CLAUDE_LOGIN_ARGV` | `providers/claude-sdk/login/scrape.ts` | Reading the CLI's login output — the part most likely to break on a CLI release, kept pure so it needs no binary to test. Strips ANSI, requires a `state` before calling a URL complete (a chunked stream delivers half of one), and refuses a paste that is not exactly `code#state`. The pinned subcommand and its provenance live here and nowhere else |
